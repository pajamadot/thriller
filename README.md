...
| 命令 | 功能 | 输出 |
|------|------|------|
| `/开始` | 项目初始化（类型、篇幅、视角、诡计类型、风格、主题） | `.thriller-state.json` |
| `/主题` | 主题前提与立意（Egri 前提法） | `theme.md` |
...
```
诡计优先（本格/密室/叙述诡计）：
  /开始 → /主题 → /诡计设计 → /人物 → /结构 → /场景 → /改稿 → /揭示 → /导出

角色优先（社会派/心理悬疑）：
  /开始 → /主题 → /人物 → /诡计设计 → /结构 → /场景 → /改稿 → /揭示 → /导出

结构优先（惊悚/犯罪程序）：
  /开始 → /主题 → /结构 → /人物 → /诡计设计 → /场景 → /改稿 → /揭示 → /导出
```
...
```
线性完稿 → /互动初始化(来源:已有小说) → 识别决策点 → /分支架构
  → /选择设计 → /角色分支 → /状态系统 → /节点 1..N → /结局
  → /一致性检查 → /导出 {格式}
```
...
| 命令 | 功能 | 触发时机 |
|------|------|----------|
| `/复盘` | 项目级回顾（流程偏差、产出评价、修正建议） | 每个项目完成后 |
| `/进化` | 跨项目模式识别 → 假设形成 → 方法论修改 | 积累 2+ 个复盘后 |
| `/对标` | 外部方法论批判性对标与整合 | 接触新知识时 |
| `/解剖` | 优秀作品逆向工程 | 分析优秀作品时 |
| `/盲区` | 认知盲区扫描（5种盲区类型检测） | 每 3-5 个项目后 |
| `/脉搏` | 创作生态监测（趋势、平台、读者变化） | 持续 |
