...
const DEFAULT_PLAN_DIR = path.join(ROOT, 'storyboard', 'specs', 'storyboard-plan');
...
function loadStoryboardPlan(planDir = DEFAULT_PLAN_DIR) {
  const absolutePlanDir = path.resolve(planDir);
  const manifestPath = path.join(absolutePlanDir, 'manifest.json');
  const manifest = readJson(manifestPath);
  const slots = readJson(path.join(absolutePlanDir, 'slots.json'))
    .slice()
    .sort((left, right) => left.slot - right.slot);
  const bandsDir = path.join(absolutePlanDir, 'bands');
  const bands = fs
